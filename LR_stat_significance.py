...
Number_of_Simulations = 1000 # 1000
NUM_ITERATION = 50
FIRST_TIME_FLAG = 0
DEBUG_flag = True
...
if FIRST_TIME_FLAG:
    ...
    #exclude_GATids = np.load('exclude_GATids.npy', allow_pickle=True) 
    #unique_place_ids = [locid for locid in unique_place_ids if locid not in exclude_GATids]
    ...
    for group_name, df_group in grouped_data:
        ...
        for row_index, row in df_group.iterrows():
            loc = row['loc']
            if loc in unique_place_ids:
                loc_indx = np.where(unique_place_ids==loc)[0][0]
                start = row['start']
                end = row['end']
                weighted_time_in_month =  row['weight'] * (end - start).days/30.0
...
#            ci = (len(where_greater[0])*100.0)/(Number_of_Simulations)
    ...
    #Analysis and debug
    if DEBUG_flag:
        cases_per_loc = [0 for k in unique_place_ids]
        controls_per_loc = [0 for k in unique_place_ids]
        grouped_data = data.groupby('loc')
        for group_name, df_group in grouped_data:
            status_dict = {}
            identifier = df_group.iloc[0]['loc']
            example_idndx = np.where(unique_place_ids==identifier)[0][0]        
            for row_index, row in df_group.iterrows(): 
                if row['ID'] not in status_dict:
                    status = row['status']
                    if status == 'CASE':
                        case_status = 1
                        cases_per_loc[example_idndx] = cases_per_loc[example_idndx] + 1
                    if status == 'CONTROL':
                        case_status = 0  
                        controls_per_loc[example_idndx] = controls_per_loc[example_idndx] + 1
                    status_dict[row['ID']] = case_status
                    
        tract_risks['Cases'] =  cases_per_loc
        tract_risks['Controls'] =  controls_per_loc
        smoothed_ratio = [float(cases_per_loc[i])/(controls_per_loc[i] + 1) for i in range(len(controls_per_loc))]
        tract_risks['smoothed_ratio'] =  smoothed_ratio
        #tract_risks= tract_risks[tract_risks['Controls'] > 20]
        tract_risks.to_csv('tract_risks_simulations_withCI.csv') 


#low_control_count = tract_risks[tract_risks['Controls'] <= 20]
#exclude_GATids = []
#for idx in low_control_count.index:
#    exclude_GATids.append(idx)
#
#np.save('exclude_GATids.npy', exclude_GATids) 